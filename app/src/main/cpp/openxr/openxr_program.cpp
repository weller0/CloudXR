...
struct OpenXrProgram : IOpenXrProgram {
    ...
    ~OpenXrProgram() override {
        ...
        Log::Write(Log::Level::Info,Fmt("xrDestroySwapchain start"));
        for (Swapchain swapchain : m_swapchains) {
            Log::Write(Log::Level::Info,Fmt("xrDestroySwapchain handle(%p)",swapchain.handle));
    ...
    void LogReferenceSpaces() {
        ...
        for (XrReferenceSpaceType space : spaces) {
            Log::Write(Log::Level::Verbose, Fmt("XrReferenceSpaceType  Name: %s", to_string(space)));
        }
    }
    ...
    void HandleSessionStateChangedEvent(const XrEventDataSessionStateChanged& stateChangedEvent, bool* exitRenderLoop,
        ...
        switch (m_sessionState) {
            case XR_SESSION_STATE_READY: {
                ...
                sessionBeginInfo.next = NativeWindow;//need set NativeWindow
    ...
    bool RenderLayer(XrTime predictedDisplayTime, std::vector<XrCompositionLayerProjectionView>& projectionLayerViews,
        ...
        for(XrView xrView : m_views) {
            Log::Write(Log::Level::Info, Fmt("views fov[%f,%f,%f,%f], pos:[%f,%f,%f,%f],[%f,%f,%f]",
        ...
        for (auto hand : {Side::LEFT, Side::RIGHT}) {
            ...
            if (XR_UNQUALIFIED_SUCCESS(res)) {
                if ((spaceLocation.locationFlags & XR_SPACE_LOCATION_POSITION_VALID_BIT) != 0 &&
                    ...
                    Log::Write(Log::Level::Info,Fmt("XR_TYPE_ACTION_SPACE_CREATE_INFO pos:{(%f,%f,%f,%f),(%f,%f,%f)}",
                                                    spaceLocation.pose.orientation.x,spaceLocation.pose.orientation.y,spaceLocation.pose.orientation.z,
                                                    spaceLocation.pose.orientation.w,spaceLocation.pose.position.x,spaceLocation.pose.position.y,spaceLocation.pose.position.z));
